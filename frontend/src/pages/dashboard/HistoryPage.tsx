...
export function HistoryPage({ items, onSetActiveCode, onDelete }: HistoryPageProps) {
  ...
  return (
    <section className="rounded-[28px] border border-white/8 bg-[#151515] p-5">
      <div className="mb-5">
        <p className="text-xs uppercase tracking-[0.22em] text-slate-500">历史记录</p>
        <h3 className="mt-2 text-2xl font-semibold text-white">最近访问与分析轨迹</h3>
      </div>
      <div className="max-h-[620px] space-y-3 overflow-y-auto overscroll-contain pr-1">
        {items.map((item) => (
          <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/8 bg-[#0f0f0f] px-4 py-4">
            <div>
              <div className="font-medium text-white">{item.stock_name} ({item.stock_code})</div>
              <div className="mt-1 text-sm text-slate-500">{item.visit_time_str}</div>
            </div>
