    ...
    <section className="rounded-[30px] border border-white/8 bg-[radial-gradient(circle_at_top_left,rgba(22,93,255,0.14),transparent_22%),#111827] p-6">
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-black text-white xl:text-5xl">{activeStock ? `${activeStock.display_name} · ${activeStock.full_symbol}` : "请先完成选股配置"}</h1>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-400">
            ...
            {activeStock ? <span className="rounded-full border border-white/8 bg-white/4 px-3 py-2">市场 A股</span> : null}
          </div>
        </div>
