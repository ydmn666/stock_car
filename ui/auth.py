...
import time
from modules.data_loader.loader import login_user, register_user, get_user_history, delete_history_item, \
    delete_all_user_history


# ==============================================================================
# 1. 定义模态窗口 (Dialogs) - 这是官方最简单的弹窗写法
# ==============================================================================

@st.dialog("⚠️ 退出确认")
def show_logout_dialog():
    st.write("确定要退出当前的账号吗？")
...
def render_user_header():
    ...
    with c_user:
        with st.container(border=True):
            ...
            # A. 用户名
            with cols[0]:
                st.markdown(f"**👤 {st.session_state.get('current_user', 'Guest')}**")

            # B. 历史记录
            with cols[1]:
                with st.popover("🕒", help="浏览历史"):
                    st.markdown("#### 📜 查阅记录")
                    user = st.session_state.get('current_user')
                    if user:
                        history = get_user_history(user)
                        if history:
                            # 点击这个按钮，直接调用上面的 show_clear_history_dialog 函数
                            if st.button("🗑️ 清空所有", type="primary", use_container_width=True):
                            ...
                            for item in history:
                                ...
                                with h_col2:
                                    if st.button("✕", key=f"del_hist_{rec_id}", help="删除此条"):
            ...
            # C. 退出按钮
            with cols[2]:
                # 点击这个按钮，直接调用 show_logout_dialog 函数
                if st.button("🚪", help="退出登录"):
                    show_logout_dialog()


# ==============================================================================
# 3. 背景与登录页逻辑 (保持你喜欢的样式不变)
# ==============================================================================
...
def render_login_ui():
    """渲染登录流程"""
    set_bg_hack_url()
    if "enter_system_clicked" not in st.session_state: st.session_state.enter_system_clicked = False
    ...
    if not st.session_state.enter_system_clicked:
        ...
        st.markdown("<div class='cover-subtitle'>新能源汽车股票分析系统</div>", unsafe_allow_html=True)
            ...
            with tab_reg:
                ...
                if st.button("注册并登录", use_container_width=True, type="primary"):
                    if new_user and new_pwd:
                        ...
                        if success:
                            st.success(
                                msg); st.session_state.is_logged_in = True; st.session_state.current_user = new_user; time.sleep(
                                0.5); st.rerun()
                        else:
                            st.error(msg)
                    else:
